...
class CodexOrchestrationService:
    ...
    def train_all(
        ...
        *,
        ...
        logger.info(
            "Running %d Codex training cycles across %s layers and %s features",
            cycles,
            "override" if layers else "configured",
            "override" if features else "configured",
        )
        before = len(self._trainer.history)
        self._trainer.train_for_cycles(cycles, layers=layers, features=features)
