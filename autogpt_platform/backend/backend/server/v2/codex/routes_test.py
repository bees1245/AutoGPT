...
def test_train_all_endpoint_accepts_overrides(
    ...
    assert payload["cycles"] == 1
    assert len(payload["records"]) == 1
    assert recording_agent.run_calls[0].startswith("Improve book for measurement")


def test_train_all_endpoint_rejects_invalid_overrides(client: TestClient) -> None:
    response = client.post(
        "/codex/train-all",
        json={"cycles": 1, "layers": ["cubit"]},
    )

    assert response.status_code == 400
    assert "unknown layers" in response.json()["detail"]
